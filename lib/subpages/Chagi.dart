...
class Chagi extends StatefulWidget {
  @override
  _ChagiState createState() => _ChagiState();
}

class _ChagiState extends State<Chagi> {
  @override
  int currentPage = 0;
  ...
  initiateSearch(value) {
    ...
    if (queryResultSet.length == 0 && value.length == 1) {
      ...
      queryResultSet.forEach((element) {
        if (element['kick'].startsWith(capitalizedValue)) {
          setState(() {
...
class _ListPageState extends State<ListPage> {
  Future getPosts() async {
    QuerySnapshot qn =
    await Firestore.instance.collection('Kicks').getDocuments();
    return qn.documents;
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: Container(
        ...
        child: Column(
          children: <Widget>[
            Navbar(magia:  widget.post.data['kick'],),
            Text(
              widget.post.data['kick'],
              style: textfirst,
            ),
            Text(
