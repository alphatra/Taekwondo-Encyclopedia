...
class Jireugi extends StatefulWidget {
  @override
  _JireugiState createState() => _JireugiState();
}

class _JireugiState extends State<Jireugi> {
  @override
  int currentPage = 0;
  ...
  initiateSearch(value) {
    ...
    if (queryResultSet.length == 0 && value.length == 1) {
      ...
      queryResultSet.forEach((element) {
        if (element['Strike'].startsWith(capitalizedValue)) {
          setState(() {
...
class _ListPageState extends State<ListPage> {
  Future getPosts() async {
    QuerySnapshot qn =
    await Firestore.instance.collection('Strikes').getDocuments();
    return qn.documents;
  }
                    ...
                    return Padding(
                      ...
                      child: GestureDetector(
                        ...
                        child: RichText(
                          textAlign: TextAlign.start,
                          text: TextSpan(
                            text: (snapshot.data[index]['Strike']) + "\n",
                            style: textfirst,
                            children: <TextSpan>[
                              TextSpan(
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: Container(
        ...
        child: Column(
          children: <Widget>[
            Navbar(magia:  widget.post.data['Strike'],),
            Text(
              widget.post.data['Strike'],
              style: textfirst,
            ),
            Text(
